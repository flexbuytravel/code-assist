...
app.post("/", async (req, res) => {
  try {
    const { companyId, origin } = req.body;
    if (!companyId) {
      return res.status(400).json({ error: "Company ID is required." });
    }

    const stripeClient = new Stripe(process.env.FLEXWAVE_STRIPE_SECRET_KEY!, {
    ...
    const returnUrlBase = origin || "http://localhost:9003";
...
export const connect_redirect = https.onRequest(app);
