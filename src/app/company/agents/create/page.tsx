  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "agents", agentId), {
        ...
        companyId: "TODO: set from logged-in company",
      });

      router.push("/company/agents");
    } catch (error: any) {
      console.error(error);
      setErrors({ firebase: error.message || "Failed to create agent." });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-md">
        <h2 className="mb-6 text-center text-2xl font-bold">Create Agent</h2>
        {errors.firebase && (
